...
class PipedriveController extends Controller {
                ...
                if (!$deal['f9717f095c375ebfc91312429b54821df8972fb3']) {
                    ...
                    if (!$address) {
                        ...
                        if (!$subscription) {
                            ...
                            if ($deal['0f564b10f66aa6eba1294de86c0ffcb670039947_locality']) {
                                $city = City::where('name', 'LIKE', '%' . $deal['0f564b10f66aa6eba1294de86c0ffcb670039947_locality'] . '%')->first(['id']);
                                if ($city) {
                        ...
                        $subscription = Subscription::where('name', $deal['title'])->first();       
                        if ($subscription && $deal['0f564b10f66aa6eba1294de86c0ffcb670039947_locality'] && !$subscription->city_id) {
                            $city = City::where('name', 'LIKE', '%' . $deal['0f564b10f66aa6eba1294de86c0ffcb670039947_locality'] . '%')->first(['id']);
                            if ($city) {
                                $subscription->city_id = $city->id;
                            }
    ...
    public function getTrial() {
        $deals = file_get_contents('https://api.pipedrive.com/v1/pipelines/3/deals?filter_id=1&api_token=' . Config('constants.pipedriveApiToken'));
        $deals = json_decode($deals, true);
//        Controlsler::pr($deals);
        $count = 0;
        if (isset($deals['data'])) {
            $usercount = 0;
            $updatecount = 0;
            $flashMessage = "";
            foreach ($deals['data'] as $key => $deal) {
                if ($deal['f9717f095c375ebfc91312429b54821df8972fb3'] && $deal['stage_id'] == 13) {
                    ...
                    if (!$address) {
                        ...
                        if (!$subscription) {
                            ...
                            if ($deal['0f564b10f66aa6eba1294de86c0ffcb670039947_locality']) {
                                $city = City::where("is_active", 1)->where('name', 'LIKE', '%' . $deal['0f564b10f66aa6eba1294de86c0ffcb670039947_locality'] . '%')->first(['id']);
                                if ($city) {
                                    $subscription->city_id = $city->id;
                                }
                            }
